...
/* ══════════════════════════════════════════
   STATUS META — كل حالة ليها label + badge + notesKey
══════════════════════════════════════════ */



/* حالات الموظف التي تحمل ملاحظات — بالترتيب الزمني */
...
const getMeta = (status) =>
  ...
  const entries = EMPLOYEE_STATUSES
    .map(s => {
      ...
      return note ? { status: s, note } : null;
    })
    .filter(Boolean);
  ...
  /* ── approve — يخزن كل حاجة في requests فقط ── */
    ...
    try {
      ...
      setRequests(prev => prev.map(r =>
        r.id === selectedRequest.id
          ? { ...r, status: 'approved', employeeId: selectedEmployeeId, employeeName: chosenEmp?.name || '' }
          : r
      ));
  ...
  const handleReject = async () => {
    setSubmitting(true);
    try {
      await updateDoc(doc(db, 'requests', selectedRequest.id), { status: 'rejected', rejectedAt: new Date() });
                ...
                <div className={`${svcColor.bg} ${svcColor.border} border-b px-5 py-3 flex items-center justify-between`}>
                  ...
                  <div className="flex items-center gap-2">
                    {request.employeeName && (
                      <span className="text-xs px-2.5 py-1 flex items-center gap-1 text-gray-500 bg-white/80 px-2 py-0.5 rounded-lg border border-gray-100">
                        <User className='w-4 h-4 ' /> {request.employeeName}
                      </span>
                    )}
      ...
      {openApprovalModal && (
        <Modal onClose={() => setOpenApprovalModal(false)}>
          <div className="flex flex-col gap-2">
            <h1 className="font-bold text-xl px-2 text-blue-950 border-r-4 border-r-orange-600 py-1">الموافقة على الطلب</h1>
            ...
            <div className="bg-blue-50 py-3 px-4 border-dashed border-blue-300 border rounded-lg">
              ...
              <div className="flex items-center justify-between mt-3 flex-wrap gap-3">
                <div className="flex flex-col items-start">
                  <h3 className="text-gray-500 text-xs"><User className="w-3.5 h-3.5 inline-block ml-1" />اسم العميل</h3>
                  <p className="font-semibold text-sm text-blue-950 mt-0.5">{selectedRequest.name}</p>
                </div>
                <div className="flex flex-col items-start">
                  <h3 className="text-gray-500 text-xs"><Phone className="w-3.5 h-3.5 inline-block ml-1" />رقم الهاتف</h3>
                  <p className="font-semibold text-sm text-blue-950 mt-0.5">{selectedRequest.phoneNumber}</p>
                </div>
                <div className="flex flex-col items-start">
                  <h3 className="text-gray-500 text-xs"><MapPin className="w-3.5 h-3.5 inline-block ml-1" />الموقع</h3>
                  <p className="font-semibold text-sm text-blue-950 mt-0.5">{selectedRequest.location || '—'}</p>
                </div>
                <div className="flex flex-col items-start">
                  <h3 className="text-gray-500 text-xs"><Home className="w-3.5 h-3.5 inline-block ml-1" />المساحة</h3>
            ...
            <div className="flex flex-col items-start mt-3">
              <label className="font-semibold mb-1 text-sm">اختر الموظف المسؤول <span className="text-red-500">*</span></label>
              <select
                value={selectedEmployeeId}
                onChange={e => setSelectedEmployeeId(e.target.value)}
                className="w-full border border-gray-300 rounded-lg p-2.5 mt-1 text-sm focus:ring-2 focus:ring-[#f2a057]/30 focus:border-[#f2a057] outline-none bg-white">
              ...
              {employees.length === 0 && (
                <p className="text-xs text-red-400 mt-1">لا يوجد موظفون متاحون، يرجى إضافة موظفين أولاً</p>
              )}
            </div>
            ...
            <div className="flex gap-2 mt-3 justify-end">
              ...
              <button onClick={handleApprove} disabled={!selectedEmployeeId || submitting}
      ...
      {openRejectModal && (
        <Modal onClose={() => setOpenRejectModal(false)}>
          <div className="flex flex-col gap-4">
            <h1 className="font-bold text-xl px-2 text-blue-950 border-r-4 border-r-red-500 py-1">رفض الطلب</h1>
            <hr className="text-gray-200 rounded-full w-full" />
            ...
            <p className="text-sm text-gray-600 text-center">
              هل أنت متأكد من رفض هذا الطلب؟ سيتم تغيير حالته إلى <span className="font-bold text-red-600">مرفوض</span>.
            </p>
