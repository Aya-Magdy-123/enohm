...
import {
  collection, doc, updateDoc, orderBy, query,
  serverTimestamp, addDoc, where,
  getCountFromServer, getDocs, startAfter, limit,
...
function EmployeeRequests() {
  ...
  const [tabCounts, setTabCounts] = useState({
    ...
    contacted: 0, inProgress: 0, completed: 0,
  });
  ...
  const fetchCounts = useCallback(async (uid) => {
    ...
    try {
      ...
      const statuses = ['pending', 'approved', 'contacted', 'inProgress', 'completed'];

      const [allSnap, ...statusSnaps] = await Promise.all([
        getCountFromServer(query(collection(db, 'requests'), base)),
        ...statuses.map(s =>
          getCountFromServer(query(collection(db, 'requests'), base, where('status', '==', s)))
        ),
      ]);
  ...
  /** جيب صفحة */
  const fetchPage = useCallback(async (uid, page, tab, svcFilter, stFilter, cursorsMap) => {
    if (!uid) return;
    setLoading(true);
    try {
  ...
  const goToPage = (page) => {
    ...
    setCurrentPage(page);
    fetchPage(currentUser.uid, page, activeTab, serviceFilter, statusFilter, cursors);
  };
  ...
  const stats = [
    { title: t('employeeRequests.stats.total'),      value: tabCounts.all,        textColor: 'text-blue-500',  bgLight: 'bg-blue-50' },
    { title: t('status.pending'),                    value: tabCounts.pending,    textColor: 'text-gray-500',  bgLight: 'bg-gray-50' },
    { title: t('employeeRequests.stats.inProgress'), value: tabCounts.inProgress, textColor: 'text-amber-500', bgLight: 'bg-amber-50' },
    { title: t('employeeRequests.stats.completed'),  value: tabCounts.completed,  textColor: 'text-green-500', bgLight: 'bg-green-50' },
  ];

  const tabs = [
    { key: 'all',        label: t('employeeRequests.tabs.all'),        count: tabCounts.all },
    { key: 'pending',    label: t('status.pending'),                   count: tabCounts.pending },
    { key: 'contacted',  label: t('employeeRequests.tabs.contacted'),  count: tabCounts.contacted },
    { key: 'approved',   label: t('status.approved'),                  count: tabCounts.approved },
    { key: 'inProgress', label: t('employeeRequests.tabs.inProgress'), count: tabCounts.inProgress },
    { key: 'completed',  label: t('employeeRequests.tabs.completed'),  count: tabCounts.completed },
  ];
            ...
            return (
              <div key={request.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-300">
                ...
                <div className="px-5 pb-4 flex justify-end gap-2">
                  ...
                  {!isPending && request.status !== 'completed' &&  request.status !== 'pendingForCompleted' && request.status !== 'rejected' && (
        ...
        <Modal onClose={() => setOpenRejectModal(false)}>
          <div className="flex flex-col gap-4">
            ...
            {rejectingRequest.approvedByAdmin === true ? (
              ...
              <p className="text-sm text-gray-600 text-center">{t('lastUpdatePendingAdmin')}</p>
            )}
